...
from resources.utils import email_sender
...
@router.get("/token1/")
async def token1(token: str = Depends(oauth2_scheme)):
    data = {
        'data': 'maha'
    }
    return data
...
def get_user(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user:
        ...
        return HTTPException(detail="User does not logged in")
...
def forget_password(reset_password: schemas.ResetPassword, email:str, db: Session = Depends(get_db)):
    email = email.lower()
    email_response = email_sender(email)
